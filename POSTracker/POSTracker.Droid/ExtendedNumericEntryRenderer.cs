...
[assembly: ExportRenderer(typeof(ExtendedNumericEntry), typeof(ExtendedNumericEntryRenderer))]
namespace XLabs.Forms.Controls
{
	/// <summary>
	/// Class ExtendedEntryRenderer.
	/// </summary>
	public class ExtendedNumericEntryRenderer : EntryRenderer
	{
		...
		protected override void OnElementChanged(ElementChangedEventArgs<Entry> e)
		{
			...
			var view = (ExtendedNumericEntry)Element;
		...
		{
			var element = (ExtendedNumericEntry)this.Element;
					...
					if (Math.Abs(deltaX) > Math.Abs(deltaY))
					{
						if (Math.Abs(deltaX) > MinDistance)
						{
							if (deltaX < 0)
							{
								...
								//FIXME:	element.OnRightSwipe(this, EventArgs.Empty);
								return;
							}

							if (deltaX > 0)
							{
								//FIXME:element.OnLeftSwipe(this, EventArgs.Empty);
								return;
							}
						}
						else
		...
		protected override void OnElementPropertyChanged(object sender, PropertyChangedEventArgs e)
		{
			...
			if (e.PropertyName == ExtendedNumericEntry.FontProperty.PropertyName)
			{
				SetFont(view);
			}
			else if (e.PropertyName == ExtendedNumericEntry.XAlignProperty.PropertyName)
			{
				SetTextAlignment(view);
			}
			else if (e.PropertyName == ExtendedNumericEntry.HasBorderProperty.PropertyName)
			{
				//return;   
			}
			else if (e.PropertyName == ExtendedNumericEntry.PlaceholderTextColorProperty.PropertyName)
			{
				SetPlaceholderTextColor(view);
			}
			else
		...
		/// </summary>
		/// <param name="view">The view.</param>
		private void SetTextAlignment(ExtendedNumericEntry view)
		{
			switch (view.XAlign)
			{
		...
		/// </summary>
		/// <param name="view">The view.</param>
		private void SetFont(ExtendedNumericEntry view)
		{
			if (view.Font != Font.Default)
			{
				Control.TextSize = view.Font.ToScaledPixel();
				//FIXME:	Control.Typeface = view.Font.ToExtendedTypeface(Context);
			}
		}
		...
		/// </summary>
		/// <param name="view">The view.</param>
		private void SetPlaceholderTextColor(ExtendedNumericEntry view)
		{
		...
		/// </summary>
		/// <param name="view">The view.</param>
		private void SetMaxLength(ExtendedNumericEntry view)
		{
